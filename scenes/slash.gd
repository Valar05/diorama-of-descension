...
@export var airborne_attack_elevation_add := 8.0
...
func _process(_delta):
	...
	var airborne := false
	var air_bonus := airborne_attack_elevation_add
	if parent_player and ("elevation" in parent_player):
		airborne = parent_player.elevation > 0.0
		if "airborne_attack_elevation_add" in parent_player:
			air_bonus = float(parent_player.airborne_attack_elevation_add)
	if active:
				...
				if is_dash_slash and target_parryable:
					...
					if target.has_method("take_damage"):
						...
						if airborne and parent_player and "elevation_bonus" in parent_player:
							parent_player.elevation_bonus += air_bonus
					applied = true
				if not applied:
					if hs:
